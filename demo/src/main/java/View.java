...
public class View extends Div {
    ...
    public View() {
        ...
        Span result = new Span("Left: 0.0 px, top: 0.0 px");

        Button buttonRead = new Button("Read pos", event -> {
            gridScroller.readScrollPositions(scrollPosition -> result.setText("Left: " +scrollPosition.getLeft()+ " px, top: "+scrollPosition.getTop()+" px"));
        });
        buttonRead.setId("read");

        add(new HorizontalLayout(numberField, buttonLeft, buttonTop, buttonBoth, buttonRead, result), grid);
    }
}
